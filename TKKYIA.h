...
// needs wire, spi, adafruit_gfx, adafruit_ssd1306
...
class Ultrasensor {
  public:
    Ultrasensor(byte trigger, byte echo, unsigned short maxdistcm = 400, unsigned long maxtimeoutmicros = 0);

    float getdistcm();
    float getdistcm(float tempkelvin);

  private:
    byte trigger, echo;
    unsigned short maxdistcm;
    unsigned long maxtimeoutmicros;
};
...
class I2Cdev {
    ...
    class TwoWire {
        ...
        public:
            ...
            uint8_t end(uint16_t timeout=0);
